...
function floodFill(canvas, x, y, target) {
    ...
    if (x - 1 >= 0 && canvas[x - 1][y] == canvas) {
        floodFill(canvas, x - 1, y, target);
    }
    if (x + 1 < canvas.length && canvas[x + 1][y] == canvas) {
        floodFill(canvas, x + 1, y, target);
    }
    if (y - 1 >= 0 && canvas[x][y - 1] == canvas) {
        floodFill(canvas, x, y - 1, target);
    }
    if (y + 1 < canvas[x].length && canvas[x][y + 1] == canvas) {
        floodFill(canvas, x, y + 1, target)
    }
    return canvas;
}
